    ...
    <?php
        $credit_limit = \Auth::guard('customer')->user()->remaining_credit_limit;
    ...
    <div class="pos-data-table p-3">
        <dl class="row">
            ...
            <dd class="col-6 text-right border-top font-weight-bold pt-2" id="total-price">{{Helpers::set_symbol(round($total+$totalTax+$addonTotalTax+$deliveryCharge, 2)) }}</dd>
        </dl>

        <form action="{{route('customer.pos.order')}}" id='order_place' method="post">
            @csrf
            ...
            <div class="pt-4 mb-4">
                ...
                <ul class="list-unstyled option-buttons">
                    <li id="cash_payment_li" style="display: {{ session('order_type') != 'home_delivery' ?  'block' : 'none' }}">
                        ...
                        <label for="cash" class="btn btn-bordered px-4 mb-0">{{translate('Pay')}}</label>
                    </li>
                    <li id="card_payment_li" style="display: {{ session('order_type') != 'home_delivery' ?  'block' : 'none' }}">
                        <input type="radio" value="credit_limit" id="credit_limit" name="type" hidden="">
                        <label for="credit_limit" class="btn btn-bordered px-4 mb-0">{{translate('Credit')}}</label>
                    </li>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{route('customer.pos.discount')}}" method="post" class="row mb-0">
                        @csrf
    ...
    <div class="modal fade" id="add-tax" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{route('customer.pos.tax')}}" method="POST" class="row">
                        @csrf
    ...
    <script>
        ...
        $("input[name='type']").change(function(event){
            if($(this).val() == 'credit_limit'){
                ...
                let credit = parseInt("{{$credit_limit}}");
                let total = parseInt("{{round($total+$totalTax+$addonTotalTax+$deliveryCharge, 2)}}");

                if(total > credit){
                    toastr.info("Product price is exceeded the credit limit. Please contact to Administrator.");
                }
            }
        });
    </script>
