                            ...
                            <div class="card-body pt-0" id="items">
                                <div class="pos-item-wrap justify-content-center">
                                    ...
                                    @php
                                        ...
                                        @include('admin-views.pos._single_product',['product'=>$product,"price"=>$price])
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="p-2 p-sm-4">
                                <div class="form-group d-flex gap-2">
                                    <select name="customer_id" data-placeholder="{{translate('Walk_In_Customer')}}" class="js-data-example-ajax form-control form-ellipsis customer">
                                        ...
                                        @foreach(\App\User::select('id', 'f_name', 'l_name','credit_limit','remaining_credit_limit')->get() as $customer)
                                            <option data-id="{{ $customer['credit_limit'] }}" data-remain="{{ $customer['remaining_credit_limit'] }}" value="{{$customer['id']}}" {{ session()->get('customer_id') == $customer['id'] ? 'selected' : '' }}>{{$customer['f_name']. ' '. $customer['l_name'] }}</option>
                                        @endforeach
                                    </select>
...
@push('script_2')
    ...
    <script>
        ...
        $('.quick-view-trigger').click(function() {
            ...
            let sess = "{{session()->has('customer_id')}}";
            if(!sess){
                return toastr.error("Please select customer");
