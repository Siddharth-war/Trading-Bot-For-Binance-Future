...
class SystemController extends Controller
{
    ...
    {
        return view('vendor-views.settings');
    }
    ...
    public function settingsUpdate(Request $request): RedirectResponse
    {
        $request->validate([
            'f_name' => 'required',
            'email' => ['required', 'unique:users,email,' . auth('vendor_users')->id() . ',id'],
            'phone' => 'required',
        ], [
            'f_name.required' => translate('First name is required!'),
        ]);

        $vendor = $this->vendor->find(auth('vendor_users')->id());
        $vendor->first_name = $request->f_name;
        $vendor->last_name = $request->l_name;
        $vendor->email = $request->email;
        $vendor->phone_number = $request->phone;
        $vendor->image = $request->has('image') ? Helpers::update('vendor/', $vendor->image, 'png', $request->file('image')) : $vendor->image;
        $vendor->save();

        Toastr::success(translate('Vendor updated successfully!'));
        return back();
    }
    ...
    public function settingsPasswordUpdate(Request $request): RedirectResponse
    {
        ...
        Toastr::success(translate('Vendor password updated successfully!'));
        return back();
    }
    ...
    public function appActivate(Request $request, $app_id): Renderable
    {
        ...
        return view('vendor-views.app-activation', compact('app_id', 'app_name', 'app_link'));
    }
